    ...
    public function transactionHistory(Request $request)
    {
        ...
        $formattedTransactions = $transactions->map(function ($transaction) use ($user, $userType) {
            ...
            return [
                ...
                'sender' => [
                    'id' => $sender->id ?? null,
                    'type' => $transaction->sender_type ?? null,
                    'name' => $sender->name ?? null,
                    'phone'=>$sender->phone ?? null,
                    'phonecode'=>$sender->phonecode ?? null,
					'profile_image' => !empty($sender->profile_image) ? url('public/' . $sender->profile_image) : null,
                ],
                'receiver' => [
                    'id' => $receiver->id ?? null,
                    'type' => $transaction->receiver_type ?? null,
                    'name' => $receiver->name ?? null,
                    'phone'=>$receiver->phone ?? null,
                    'phonecode'=>$receiver->phonecode ?? null,
    				'profile_image' => !empty($receiver->profile_image) ? url('public/' . $receiver->profile_image) : null,
                ],
